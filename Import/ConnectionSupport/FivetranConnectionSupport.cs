...
public class FivetranConnectionSupport : IConnectionSupport
{
    ...
    public void RunImport(object? connection)
    {
        ...
        var mappings = new List<string> { "Lineage mappings:" };
        foreach (var connector in connectors)
        {
            ...
            foreach (var schema in connectorSchemas?.Schemas ?? [])
            {
                ...
                {
                    mappings.Add($"  {connector.Id}: {schema.Key}.{table.Key} -> {schema.Value?.NameInDestination}.{table.Value.NameInDestination}");
                }
            }
        }
